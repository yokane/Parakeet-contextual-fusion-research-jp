...
def permitted_license(value: str, policy: str) -> bool:
    ...
    if policy == "research":
        return True
    return "-nc-" not in normalized and not normalized.endswith("-nc") and "noncommercial" not in normalized


def filter_dataset(dataset: DatasetDict, *, license_policy: str, include_aux: bool) -> DatasetDict:
...
def render_card(
    *, repo_id: str, config_name: str, release_stats: dict[str, Any], published: DatasetDict, license_policy: str
) -> str:
    ...
    lines = [
        ...
        "tags:", "- japanese", "- asr", "- homophone", "- phoneme", "- contextual-asr", "- context-biasing", "- speech-recognition",
        "---", "", "# JP-HomophoneBench", "",
    ...
    lines.extend([
        ...
        f"HF repository: `{repo_id}`", f"Configuration: `{config_name}`", f"Publication license policy: `{license_policy}`", "",
        "## Published rows", "", "### Splits", "",
    ])
    ...
    lines.extend([
        ...
        "", "## Loading", "", "```python", "from datasets import load_dataset", f'ds = load_dataset("{repo_id}", "{config_name}")', "print(ds)", "```", "",
        "## Build summary", "", "```json", json.dumps(release_stats, ensure_ascii=False, indent=2, sort_keys=True), "```", "",
    ])
    return "\n".join(lines)


def upload_text(api: HfApi, *, repo_id: str, path_in_repo: str, text: str, message: str) -> None:
    api.upload_file(
...
def main() -> None:
    ...
    parser.add_argument("--license-policy", choices=("permissive", "research"), default="permissive")
    ...
    release_stats = json.loads(stats_path.read_text(encoding="utf-8")) if stats_path.exists() else {}
    dataset = filter_dataset(load_release(args.release_dir), license_policy=args.license_policy, include_aux=args.include_aux)
    ...
    upload_text(api, repo_id=args.repo_id, path_in_repo="README.md", text=card, message="docs: update JP-HomophoneBench dataset card")
    if stats_path.exists():
        api.upload_file(path_or_fileobj=str(stats_path), path_in_repo=f"provenance/{args.config_name}/stats.json", repo_id=args.repo_id, repo_type="dataset", commit_message="chore: upload benchmark release statistics")
    if args.schema.exists():
        api.upload_file(path_or_fileobj=str(args.schema), path_in_repo="schema/benchmark.schema.json", repo_id=args.repo_id, repo_type="dataset", commit_message="chore: upload benchmark JSON schema")
