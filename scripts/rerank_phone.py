...
def main() -> None:
    ...
    model = PhoneCTCHead(checkpoint["encoder_dim"], checkpoint["phone_vocab_size"], dropout=checkpoint.get("dropout", 0.0))
    ...
    with torch.inference_mode(), args.output.open("w", encoding="utf-8") as sink:
        for row in rows:
            states = load_states(args.feature_dir / f"{row['id']}.pt").to(args.device)
            for candidate in row["candidates"]:
                ...
                score = ctc_sequence_logprob(log_probs, phone_ids, blank_id=blank_id, length_norm_power=1.0)
            ...
            row["candidates"].sort(key=lambda item: item.get("fused_score", item.get("tdt", 0.0)), reverse=True)
